...
+function ($) {
    ...
    var _today_current_month = new Date().getMonth() + 1;
    var _today_month = _today_current_month<10?"0"+_today_current_month:_today_current_month;
    ...
    $.fn.datePicker = function (options) {
        ...
        _this.forEach(function (element, i) {
            ...
            $(element).on('click', function () {
                ...
                // 选择一个日期
                //touchend
                body.off('click').on('click','.day .now', function () {
                    var _currDay = $(this).attr('data-date');
                    $("ul.day>li.now.active").removeClass("active");
        ...
        function initDays(_step) {
            ...
            for (var b = 1; b < getDaysInMonth(d) + 1; b++) {
               ...
               if(nowDate.getDate() == b&&(nowDate.getMonth()==_month-1)&&(nowDate.getFullYear()==_year)){
                    daysItems += '<li class="now" data-date="' + _year + '-' + _month + '-' + (b >= 10 ? b : "0" + b) + '">今天</li>';
                }else if(nowDate.getDate() == b-1 &&(nowDate.getMonth()==_month-1)&&(nowDate.getFullYear()==_year)){
                    daysItems += '<li class="now today" data-date="' + _year + '-' + _month + '-' + (b >= 10 ? b : "0" + b) + '">明天</li>';
                }else{
            ...
            if (_options.type == 'reserve' && parseInt(_options.day) > 0) {
                ...
                DateDo(function (_currDay) { return new Date(_currDay) > date });
            }
            //不能选择周末
